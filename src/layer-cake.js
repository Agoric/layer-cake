...
const makeClassCake = makeCakeMaker(
  (objectInConstruction, additionalLayer = {}) =>
    defineProperties(objectInConstruction, gopds({ ...additionalLayer })),
);

// The layers must be disjoint
const makeTraitCake = makeCakeMaker(
  (objectInConstruction, additionalLayer = {}) => {
    defineProperties(
      objectInConstruction,
      gopds(freeze({ ...additionalLayer })),
    );
  },
);
